...
function mountScrollWorld(container, config) {
  ...
  function raf() {
    ...
    if (!destroyed) requestAnimationFrame(raf);
  }
  ...
  // ADAPTED: window listeners go through on() so destroy() can detach them.
  // Upstream leaks these (and the rAF loop) on an SPA route change.
  const bound = [];
  function on(type, fn, opts) { window.addEventListener(type, fn, opts); bound.push([type, fn, opts]); }

  on('pointerdown', onFirstGesture, { once: true, passive: true });
  on('touchstart', onFirstGesture, { once: true, passive: true });

  // Particles are a per-frame cost we can't afford alongside video scrubbing on a phone.
  seedParticles(particles, reduce || coarse);
  on('scroll', () => { if (!ticking) { ticking = true; requestAnimationFrame(read); } }, { passive: true });
  ...
  // ADAPTED: teardown for framework use. Stops the rAF loop, detaches listeners,
  // revokes the blob URLs, empties the container. Safe to call twice.
  function destroy() {
    if (destroyed) return;
    destroyed = true;
    for (const [type, fn, opts] of bound) window.removeEventListener(type, fn, opts);
    bound.length = 0;
    for (const seg of SEGMENTS) {
      const v = seg.video;
      if (!v) continue;
      try { v.pause(); } catch (e) {}
      // loadClip assigns the object URL straight to v.src, so that is where it lives.
...
// ADAPTED: upstream's CJS branch removed. `module` is not defined in an ES
// module, and keeping it made the bundler warn; the ESM export below supersedes it.
if (typeof window !== 'undefined') window.mountScrollWorld = mountScrollWorld;

// ADAPTED: ESM export for the Vite build (upstream ships CJS + global only).
